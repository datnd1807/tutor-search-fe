<template>
  <div>
    <template :model="listTutorUpdating">
      <el-badge :value="listTutorUpdating.length" class="item">
        <el-button size="medium" @click="dialogVisible = true"
          >Tutors update profile</el-button
        >
      </el-badge>
    </template>
    <el-dialog :visible.sync="dialogVisible" width="50%" top="20px">
      <span slot="title" class="fontSize"
        ><i class="el-icon-user"></i>List tutor update profile</span
      >
      <el-table
        :data="listTutorUpdating"
        height="350"
        style="width: 100%"
        v-loading="loading"
      >
          ...
          <template slot-scope="props">
            <ul>
              ...
              <div class="row">
                ...
                <div class="col">
                  <li>
                    Confirm Date: {{ props.row.confirmedDate | formatDate }}
                  </li>
                </div>
              </div>
              <div class="row">
              ...
              <div class="row">
                <certificate-updating
                  :data="listTutorUpdating[indexData(props.row)]"
                ></certificate-updating>
                <social-id
                  :data="listTutorUpdating[indexData(props.row)]"
                ></social-id>
...
import CertificateUpdating from "./CertificateUpdating.vue";
import SocialId from "./SocialId";
export default {
  components: { CertificateUpdating, SocialId },
  name: "dialog-tutor-updating",
  computed: {
    ...mapGetters(["getTutorsUpdating", "getNotification"]),
    listTutorUpdating() {
  ...
  methods: {
    ...
    ...mapMutations(["setTutorUpdating", "setNotification"]),
    indexData(conditionFn) {
      return this.listTutorUpdating.indexOf(conditionFn);
    },
    ...
    openDelete(row) {
      this.$confirm("Do you want to delete this tutor request?", "Warning", {
        ...
        .then(async () => {
          this.loading = true;
          const index = this.listTutorUpdating.indexOf(row);
          this.listTutorUpdating[index].confirmedDate = moment(
            new Date()
          ).format("YYYY-MM-DDTHH:mm");
          this.listTutorUpdating[index].confirmedBy = JSON.parse(
            localStorage.getItem("user")
          ).id;
          const response = await this.disableTutorUpdating(
            this.listTutorUpdating[index]
          );
          if (response.status == 204) {
            this.sendMessage(
              "Your account update failed",
              "Updating Account",
              row.email
            );
            this.$message({
    ...
    openActive(row) {
      this.$confirm("Do you want to active this tutor request?", "Warning", {
        confirmButtonText: "OK",
        cancelButtonText: "Cancel",
        type: "warning",
      })
        .then(async () => {
          ...
          const response = await this.enableTutorUpdating(
            this.listTutorUpdating[index]
          );
          if (response.status == 200) {
            this.sendMessage(
              "Your account update successful",
              "Updating Account",
              row.email
            );
            this.$message({
